            ...
            <div className="grid grid-cols-4 gap-1 mb-6 p-2 bg-white/5 rounded-xl border border-white/5">
              {agents.map((agent) => (
                <button
                  ...
                  className={cn(
                    "h-2 rounded-full transition-all duration-500 hover:scale-125 hover:z-10 outline-none focus-visible:ring-1 focus-visible:ring-indigo-500",
                  ...
                  title={agent.name}
                  aria-label={`View ${agent.name} details. Status: ${agent.status}`}
                />
              ))}
            </div>
