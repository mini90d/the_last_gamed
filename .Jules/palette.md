...
## 2026-06-06 - [Keyboard Accessibility for Hover Overlays]
**Learning:** Hover-triggered overlays (like video controls) are inaccessible to keyboard users unless they also trigger on focus. Using Tailwind's `group-focus-within` ensures the overlay remains visible when internal buttons receive focus.
**Action:** Always pair `group-hover` with `group-focus-within` for interactive overlays.
