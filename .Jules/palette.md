...
## 2025-05-15 - [Keyboard Visibility for Hover Overlays]
**Learning:** In high-density UIs, "hover-only" controls (like video player overlays) are completely inaccessible to keyboard users unless they are also triggered by focus. Relying on `group-hover` alone breaks the experience for non-mouse users.
**Action:** Use `focus-within:translate-y-0` (or similar) alongside `group-hover` on overlay containers to ensure they reveal themselves when any child element receives keyboard focus.
