...
**Learning:** Hover-only controls in modular dashboards (like video player overlays) are completely inaccessible to keyboard users unless they are also triggered by focus.
**Action:** Use Tailwind's 'group-focus-within' on the parent container to reveal interactive overlays when any child element receives focus, ensuring parity between mouse and keyboard experiences.
